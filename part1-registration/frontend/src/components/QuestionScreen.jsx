...
import { validateField, explainField, lookupGstin } from '../api/client';
...
export default function QuestionScreen({ entry, currentIndex, total, language, t, onNext, declarationText }) {
  ...
  useEffect(() => {
    ...
    if (entry.inputConfig?.dataSource === 'gstDetails' || entry.inputConfig?.dataSource === 'gstName') {
      const gstinAnswer = s.answers['gstin'];
      if (gstinAnswer) {
        lookupGstin(gstinAnswer)
          .then(data => setDynamicData(data))
          .catch(err => console.warn('Failed to lookup GSTIN:', err));
      }
    } else {
      setDynamicData({});
    }
  ...
  // Inject dynamic data for ConfirmReadonly
  const entryWithData = { ...entry };
  if (entry.inputConfig?.dataSource) {
    entryWithData.inputConfig = { ...entry.inputConfig, data: dynamicData };
  }
