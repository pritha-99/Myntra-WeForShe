 ...
 * Response: { [stateName]: [ { sellerId, brandName, brandUsp, categoryTypes, avgSellingPrice, city } ] }
 */
router.get('/sellers-grouped', async (req, res) => {
  try {
    ...
    console.error('Customer sellers-grouped error:', err.message);
 ...
 * Returns a single seller's profile details.
 */
router.get('/sellers/:sellerId', async (req, res) => {
  try {
    const seller = await Seller.findOne({ sellerId: req.params.sellerId, status: { $in: ['approved', 'submitted'] } })
    ...
    console.error('Customer seller detail error:', err.message);
...
router.get('/sellers/:sellerId/products', async (req, res) => {
  try {
    ...
    console.error('Customer products error:', err.message);
